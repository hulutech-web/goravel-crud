...
const evClickCbk = (info) => {
  ...
  if (
    listenTag(
      info.event.id,
      info.event.title,
      info.event.extendedProps.personalcourse_id,
      info.event.extendedProps.personalcourse_name,
      info.event.start,
      info.event.end,
      info.event.extendedProps.personalcourse_coverimage
    )
  ) {
