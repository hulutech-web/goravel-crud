      ...
      { dataIndex: 'action', title: '操作', key: 'action' },
        ...
        <template #bodyCell="{ column, text, record }">
          ...
          <template v-if="column.dataIndex === 'action'">
            <a-button type="primary" @click="cancelGroup(record)">
              取消约课
            </a-button>
          </template>
        </template>
...
const { showGroupschedule, updateGroupschedule, scheduleAppt } = useGroupschedule();
const { cancelGapt } = useGApt();
...
watch(openDrawer, async (newVal) => {
  if (newVal) {
    ...
    appts.value = res.Groupappointments
  } else {
...
const cancelGroup = async (record) => {
  //这里直接把record.ID删除就行
  console.log(record)
  await cancelGapt(record.ID)
  openDrawer.value = false;
  await refresh();
}
</script>
